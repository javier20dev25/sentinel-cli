...
function readSingleKeypress(): Promise<string> {
  return new Promise(resolve => {
    ...
    const cb = (data: Buffer) => {
      ...
      const b = data[0];
      if (b === 0x0d || b === 0x0a) resolve('enter');
      else if (b === 0x1b) resolve('escape');
      else if (b === 0x61 || b === 0x41) resolve('auto');
      else if (b === 0x79 || b === 0x59) resolve('enter');
      else if (b === 0x6e || b === 0x4e) resolve('escape');
      else resolve('escape');
    };
...
const permissionPrompt: ToolPermissionCallback = async (toolName, args) => {
  if (currentMode === 'auto') return true;

  const key = `${toolName}:${JSON.stringify(args)}`;
  if (permissionCache.has(key)) return true;
  ...
  const result = await readSingleKeypress();
...
async function handleSlash(input: string): Promise<boolean> {
  ...
  switch (cmd) {
    ...
    case '/tono': {
      const selected = await selectToneModal();
      if (selected) {
        const tone = getCurrentTone();
    ...
    case '/mode': {
      ...
      if (sub === 'plan') { currentMode = 'plan'; console.log(modeBanner('plan') + '\n'); return true; }
      if (sub === 'execute') { currentMode = 'execute'; console.log(modeBanner('execute') + '\n'); return true; }
      if (sub === 'auto') { currentMode = 'auto'; console.log(modeBanner('auto') + '\n'); return true; }
    ...
    case '/tools': {
      ...
      for (const t of defs) {
        ...
        if (verbose) console.log(`      ${t.description}`);
        console.log();
      }
      return true;
    }
    ...
    case '/agent': {
      ...
      if (sub === 'set') {
        ...
        if (!id) { console.log(pc.yellow('  Usage: /agent set <id>\n')); return true; }
        if (setAgent(id)) {
    ...
    case '/cli1-import': {
      ...
      if (result.imported > 0) {
        ...
        if (result.files.length > 10) console.log(pc.gray(`    ... and ${result.files.length - 10} more`));
        console.log();
      } else {
    ...
    case '/rule': {
      ...
      if (sub === 'add') {
        ...
        if (!name || !instruction) { console.log(pc.yellow('  Usage: /rule add <name> <instruction>\n')); return true; }
      ...
      if (sub === 'remove') {
        const name = parts[2];
        if (!name) { console.log(pc.yellow('  Usage: /rule remove <name>\n')); return true; }
        if (removeRule(name)) console.log(pc.green(`  [OK] Rule "${name}" removed.\n`));
        else console.log(pc.yellow(`  Rule "${name}" not found.\n`));
        return true;
      }
      if (sub === 'toggle') {
        const name = parts[2];
        if (!name) { console.log(pc.yellow('  Usage: /rule toggle <name>\n')); return true; }
        const rules = listRules();
        const rule = rules.find(r => r.name.toLowerCase() === name.toLowerCase());
        if (!rule) { console.log(pc.yellow(`  Rule "${name}" not found.\n`)); return true; }
    ...
    case '/threat': {
      ...
      if (sub === 'query') {
        ...
        if (!author) { console.log(pc.yellow('  Usage: /threat query <author>\n')); return true; }
        ...
        if (threats.length === 0) console.log(pc.gray('  No threats found for this author.\n'));
        else {
          for (const t of threats) console.log(`  #${t.id} ${t.type} — ${t.source} — ${t.detected_at}`);
          console.log();
        }
        return true;
      }
      if (sub === 'auth') {
        ...
        console.log(pc.cyan('\n  High-Risk Authors:\n'));
        if (authors.length === 0) console.log(pc.gray('  No high-risk authors recorded.\n'));
        else {
      ...
      if (sub === 'correlate') {
        const author = parts.slice(2).join(' ');
        if (!author) { console.log(pc.yellow('  Usage: /threat correlate <author>\n')); return true; }
...
export async function oracleAsk(question: string): Promise<void> {
  ...
  try {
    ...
    const { response } = await oracleChat(question, conversationHistory, provider, undefined, 'auto');
    spinner.stop();
