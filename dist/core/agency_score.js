...
exports.calculateAgencyScore = calculateAgencyScore;
...
function detectCorrelations(findings) {
    const correlations = [];
    ...
    for (const f of findings) {
        ...
        if (!fileCategories.has(file))
            fileCategories.set(file, new Set());
    ...
    Array.from(fileCategories.entries()).forEach(([file, cats]) => {
        Array.from(cats).forEach(cat => {
            if (!categoryFiles.has(cat))
                categoryFiles.set(cat, new Set());
    ...
    const ciEvasionCount = findings.filter(f => f.subcode && (f.subcode.startsWith('WF-') || f.subcode.startsWith('TOK-'))).length;
    if (ciEvasionCount >= 2) {
        correlations.push({
            description: `Multiple CI/CD risk indicators (${ciEvasionCount})`,
            bonus: -9,
            involved: findings.filter(f => f.subcode && (f.subcode.startsWith('WF-') || f.subcode.startsWith('TOK-'))).map(f => f.subcode),
        });
    }
    Array.from(fileCategories.entries()).forEach(([file, cats]) => {
        if (cats.size >= 3) {
...
function generateRecommendation(findings, drivers) {
    const steps = [];
...
function calculateAgencyScore(findings) {
