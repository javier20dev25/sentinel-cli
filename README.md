# Sentinel CLI v4 — Security Oracle Core

> Unified Terminal Security Interface · SAST Scanner · Supply Chain Shield · AI-Powered Oracle

Sentinel CLI is an open-source, local-first security platform designed for developers and security teams. It combines a powerful static analysis engine (LiteScanner) with an AI-driven Oracle assistant that can reason about threats, query GitHub, audit packages, and guide remediation — all from your terminal.
...
### 🛡️ Supply Chain Shield
Audit npm packages **without installing** — detects typosquatting, embedded secrets, hardcoded credentials, and malicious patterns before `npm install`.
...
| `sentinel check-classified` | Check staged files against classified DB |
...
- **Node.js** >= 18.0.0
- **npm** >= 9
- **Git** (for `check-classified` and GitHub tools)
