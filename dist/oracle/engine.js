...
exports.getFilteredToolDefs = getFilteredToolDefs;
...
function buildCompactSystemPrompt() {
    const essentialTools = (0, tools_1.getToolDefs)().filter(t => ['scan', 'verify-pkg', 'gh-pr-list', 'gh-pr-view', 'gh-pr-diff', 'gh-pr-comment', 'gh-repo-list', 'doctor'].includes(t.name));
...
function getFilteredToolDefs(provider) {
    const allTools = (0, tools_1.getToolDefs)();
...
function getDefaultProvider() {
    ...
    if (!key && provider !== 'ollama' && provider !== 'qwen')
        return null;
    try {
    ...
    return __awaiter(this, arguments, void 0, function* (userInput, history, provider, onBeforeToolCall, mode = 'execute') {
        ...
        while (iterations < MAX_TOOL_ITERATIONS) {
            ...
            for (const tc of response.toolCalls) {
                ...
                const rawResult = yield (0, tools_1.runTool)(tc.name, tc.arguments);
    ...
    return __asyncGenerator(this, arguments, function* oracleChatStream_1(userInput, history, provider, onBeforeToolCall, mode = 'execute') {
        ...
        while (iterations < MAX_TOOL_ITERATIONS) {
            ...
            for (const tc of pendingToolCalls) {
                ...
                const rawResult = yield __await((0, tools_1.runTool)(tc.name, tc.arguments));
                executedTools.push({ toolName: tc.name, output: rawResult });
