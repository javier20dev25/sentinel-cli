    ...
    public scanPatch(filename: string, patch: string): LiteFinding[] {
        ...
        if (/^Binary files .* differ$/m.test(patch) || /^GIT binary patch$/m.test(patch)) {
            findings.push(makeFinding(
                filename, 0, 'BINARY_CHANGE', 'DIFF-BINARY',
                'supply-chain', 'SUSPICIOUS', 'MEDIUM', 'medium', 45,
                'Binary file changed',
                'Binary file changed — content is opaque to the line scanner; a secret inside the binary would be missed.',
                'Binary diff without inspectable text lines',
            ));
        }
