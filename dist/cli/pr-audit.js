...
function runGh(args, input) {
    ...
    try {
        ...
        return (0, child_process_1.execFileSync)('gh', args, opts).trim();
    }
    catch (e) {
        return ((_a = e.stdout) === null || _a === void 0 ? void 0 : _a.trim()) || ((_b = e.stderr) === null || _b === void 0 ? void 0 : _b.trim()) || e.message;
    }
}
function correlateThreats(author) {
    try {
        const dbPath = require('path').join(process.env.HOME || process.env.USERPROFILE || '.', '.sentinel', 'threats.db');
        ...
        const authorThreatCount = authorRow ? authorRow.threat_count || 0 : 0;
        const authorRiskLevel = authorRow ? authorRow.risk_level || null : null;
        const threats = db.prepare('SELECT COUNT(*) as cnt FROM threats WHERE author = ?').get(author);
        const patternMatches = threats ? threats.cnt || 0 : 0;
        db.close();
...
function verdictAction(decision, band) {
    if (decision === 'BLOCK') {
...
function runPrAudit(options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (options.diff) {
            ...
            if (!(0, fs_1.existsSync)(options.diffFile)) {
                return {
                    scanId: '',
                    repo: options.repo,
            ...
            rawDiff = (0, fs_1.readFileSync)(options.diffFile, 'utf8');
        }
        else {
        ...
        const contentHash = (0, crypto_1.createHash)('sha256').update(rawDiff, 'utf8').digest('hex');
        const files = parseUnifiedDiff(rawDiff);
        const scanner = new lite_scanner_1.LiteScanner(new signal_vault_1.SignalVault());
        const result = yield scanner.auditPR(options.repo, options.prNumber, options.author || 'unknown', files);
        ...
        const output = {
            ...
            threatIntel,
            verdict: Object.assign(Object.assign({}, result.verdict), { summary: result.verdict.band + ' — ' + result.verdict.decision }),
            contentHash,
        };
        if (options.outputFile) {
            (0, fs_1.writeFileSync)(options.outputFile, JSON.stringify(output, null, 2), 'utf8');
        }
        if (options.comment) {
            const markdown = buildMarkdown(output);
            const tempFile = require('path').join(require('os').tmpdir(), `sentinel-pr-${options.prNumber}-${Date.now()}.md`);
            (0, fs_1.writeFileSync)(tempFile, markdown, 'utf8');
