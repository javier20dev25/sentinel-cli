...
exports.LiteScanner = void 0;
const token_classifier_1 = require("../token_classifier");
const crypto = __importStar(require("crypto"));
function r(regex, type, subcode, category, intent, severity, confidence, riskScore, title, description) {
    return { regex, type, subcode, category, intent, severity, confidence, riskScore, title, description };
}
function makeFinding(file, line, type, subcode, category, intent, severity, confidence, riskScore, title, description, snippet, evidence) {
    return {
        type, subcode, category: category,
...
class LiteScanner {
    constructor(vault) {
        this.vault = vault !== null && vault !== void 0 ? vault : {
            recordScan: () => { },
            recordSignal: () => { },
            getCorrelations: () => [],
        };
    }
    emit(file, line, rule, snippet) {
        return {
     ...
     */
    enrichTokenFindings(findings, content, filename) {
        const isWorkflow = (filename.includes('.github/workflows/') || filename.includes('.github\\workflows\\')) &&
            (filename.endsWith('.yml') || filename.endsWith('.yaml'));
        const secretTypes = ['SECRET_GITHUB_TOKEN', 'SECRET_AWS_KEY_ID', 'SECRET_STRIPE_KEY',
            'SECRET_SENDGRID_KEY', 'SECRET_SLACK_TOKEN'];
        for (const f of findings) {
            if (!secretTypes.includes(f.type))
                continue;
            const tokenValue = (0, token_classifier_1.extractTokenValue)(f.snippet, f.type);
            if (!tokenValue)
                continue;
            const classified = (0, token_classifier_1.classifyToken)(tokenValue);
            if (classified.riskScore === 0)
                continue;
            findings.push(makeFinding(f.file, f.line, 'TOKEN_RISK', 'TOK-CLASS', 'token', 'EXFILTRATION', classified.riskLevel === 'critical' ? 'CRITICAL' :
                classified.riskLevel === 'high' ? 'HIGH' :
                    classified.riskLevel === 'medium' ? 'MEDIUM' : 'LOW', 'high', classified.riskScore, `Token classified: ${classified.tokenType}`, `${classified.tokenType} — risk score: ${classified.riskScore}/100`, f.snippet));
        }
        if (!isWorkflow)
            return;
     ...
     */
    scanFileContent(filename, content) {
        const findings = [];
        const baseName = filename.split(/[/\\]/).pop() || filename;
        const lowerName = baseName.toLowerCase();
...
exports.LiteScanner = LiteScanner;
LiteScanner.RULES = [
