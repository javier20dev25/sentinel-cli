...
const SEMANTIC_PAIRS: Record<string, Array<{ target: string; label: string }>> = {
  'SEC-GITHUB-TOKEN': [
...
export function buildAgencyGraph(
  ...
  function addEdge(sourceId: string, targetId: string, type: EdgeType, confidence: number, label: string) {
  ...
  for (const [, group] of fileGroups) {
    ...
    group.sort((a, b) => (CATEGORY_ORDER[a.category] ?? 2) - (CATEGORY_ORDER[b.category] ?? 2));
  ...
  if (workflowNodes.length >= 2) {
    ...
    const wfGroups = new Map<string, GraphNode[]>();
    for (const n of workflowNodes) {
      if (!wfGroups.has(n.file)) wfGroups.set(n.file, []);
