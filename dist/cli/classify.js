 ...
 */
function installSastPreCommitHook(repoPath) {
    const hooksDir = path.join(repoPath, '.git', 'hooks');
    if (!fs.existsSync(hooksDir))
        return false;
    const hookPath = path.join(hooksDir, 'pre-commit');
...
function uninstallPreCommitHook(repoPath) {
    ...
    if (!content.includes('SENTINEL SAST PRE-COMMIT'))
        return false;
 ...
 */
function isPreCommitHookInstalled(repoPath) {
    const hookPath = path.join(repoPath, '.git', 'hooks', 'pre-commit');
    if (!fs.existsSync(hookPath))
        return false;
    const content = fs.readFileSync(hookPath, 'utf8');
...
function handleClassifiedMenu(lang, askQuestion) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        while (true) {
            ...
            const matchingKey = Object.keys(db).find(k => path.resolve(k).replace(/\\/g, '/').toLowerCase() === normalizedProj);
