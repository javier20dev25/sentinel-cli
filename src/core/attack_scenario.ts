...
interface ScenarioMatcher {
  ...
  description: string;
  impact: string;
  matchChain: (chain: GraphChain) => boolean;
}
...
function hasAny(chain: GraphChain, subcodes: string[]): boolean {
  return chain.nodes.some(n => subcodes.includes(n.subcode));
}

function hasAll(chain: GraphChain, categories: string[]): boolean {
  const cats = new Set(chain.nodes.map(n => n.category));
  return categories.every(c => cats.has(c));
}

function hasCategory(chain: GraphChain, category: string): boolean {
  ...
  for (const chain of sorted) {
    for (const matcher of MATCHERS) {
      if (matcher.matchChain(chain)) {
        ...
        if (usedChainIds.has(hashStr(key))) continue;
        usedChainIds.add(hashStr(key));

        const sev = chain.score >= 70 ? 'CRITICAL' as const
          : chain.score >= 30 ? 'HIGH' as const
          : chain.score >= 10 ? 'MEDIUM' as const
          : 'LOW' as const;
