...
class QuarantineManager {
    ...
    getQuarantinePath(packageName, version) {
    ...
    quarantinePackage(packageName, version, reason, severity) {
    ...
    releasePackage(packageName, version) {
        const entries = this.readManifest();
    ...
    isQuarantined(packageName, version) {
        const entries = this.readManifest();
        if (version) {
            return entries.some((e) => e.packageName === packageName && e.version === version);
        }
        return entries.some((e) => e.packageName === packageName);
    }
    createPlaceholder(originalPath) {
    ...
    removePlaceholder(originalPath) {
        if (fs.existsSync(originalPath)) {
