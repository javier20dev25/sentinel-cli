...
interface TokenClassifier {
  ...
  provider: string;
  baseScore: number;
  confidence: ClassifiedToken['confidence'];
}

const CLASSIFIERS: TokenClassifier[] = [
...
function getRiskLevel(score: number): ClassifiedToken['riskLevel'] {
...
export function calculateRiskScore(baseScore: number, permissions?: string[]): number {
  let score = baseScore;
  if (permissions) {
    for (const perm of permissions) {
      score += PERMISSION_SCORES[perm] || 0;
    }
  }
  return Math.max(0, Math.min(100, score));
}

function detectJWT(token: string): boolean {
  return /^eyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/.test(token);
}

function detectGenericSecret(token: string): boolean {
...
export function extractTokenValue(snippet: string, secretType: string): string | null {
  switch (secretType) {
